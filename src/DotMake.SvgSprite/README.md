...
# DotMake Svg-Sprite Library

A dotnet library for building or extracting of an SVG sprite, i.e. a `.svg` file with child `<symbol>` tags.
We also offer a dotnet tool [svg-sprite](https://www.nuget.org/packages/svg-sprite) for same purpose.
...
Install the dotnet library from [NuGet](https://www.nuget.org/).
...
- .NET Standard 2.0 and later project.  
...
Refer to [DotMake Svg-Sprite API Reference](https://dotmake.build/svg-sprite/api/) for more details.
